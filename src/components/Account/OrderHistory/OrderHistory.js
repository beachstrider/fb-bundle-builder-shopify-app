...
const OrderHistory = () => {
  ...
  const getOrdersToShow = async (token) => {
    ...
    const configData = await request(
      `${process.env.PROXY_APP_URL}/bundle-api/bundles/${subApi.data.data[0].bundle_id}/configurations`,
      {
        method: 'get',
        data: '',
        headers: { authorization: `Bearer ${token}` }
        ...
        if (subscriptionConfigContents.data.data) {
          subscriptionConfigContents.data.data.products.forEach((product) => {
            if (product.is_default === 1) {
              ...
              const shopProd = shopProducts.filter(
                (p) => Number(p.id) === Number(product.platform_product_id)
              )[0]
              thisLoopSubList.push({
                title: shopProd ? shopProd.title : 'Missing Title',
                platform_img:
                ...
                <div key={index} className={styles.contentCardWrapper}>
                  ...
                  {subscription.items ? (
                    <div className={styles.currentOrderMenu}>
                      ...
                      <Link
                        to={`/account?date=${dayjs(subscription.date)
                          .utc()
                          .format('YYYY-MM-DD')}`}
